odoo.define('field_service_checklist.atm_checklist_sign', function (require) {
"use strict";
	...
	FormController.include({
		...
		_callSignButtonAction: function(attrs, record){
			if(attrs.name == 'signature_dummy' && record.model == 'atm.checklist'){
...
//	            		}else{
//	            			self.do_notify(_t('Information'),_t("ATM Checklist "+ result[0].name +" is confirmed!, You can't add signature."));
//	            		}
	            	}
	            });
			}
		    ...
		    if(blank.toDataURL() == canvas.toDataURL()){
                ...
                if(id){
                    rpc.query({
		                model: 'atm.checklist',
		                method: 'write',
		                args: [parseInt(id),{'signature': canvas.toDataURL().split('data:image/png;base64,')[1]}],
		            }, {
		            	async: false
		            }).then(function (result) {
		            	if(result){
